...
CREATE TABLE IF NOT EXISTS public.questions (
  ...
  correct_option_indices JSONB DEFAULT '[]'::jsonb,
  correct_integer_answer NUMERIC,
...
-- Questions Policies: Anyone can view, only Admin can insert/update/delete
...
-- Attempts Policies
CREATE POLICY "Users can view own attempts" ON public.question_attempts FOR SELECT USING (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "Users can record own attempts" ON public.question_attempts FOR INSERT WITH CHECK (auth.uid() = user_id);
...
-- Study PDFs Policies
CREATE POLICY "Anyone can view study PDFs" ON public.study_pdfs FOR SELECT USING (true);
CREATE POLICY "Admins can upload study PDFs" ON public.study_pdfs FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "Admins can update study PDFs" ON public.study_pdfs FOR UPDATE USING (public.is_admin());
CREATE POLICY "Admins can delete study PDFs" ON public.study_pdfs FOR DELETE USING (public.is_admin());

-- Support Tickets Policies
CREATE POLICY "Users can insert support ticket" ON public.support_tickets FOR INSERT WITH CHECK (true);
CREATE POLICY "Users can view own support tickets" ON public.support_tickets FOR SELECT USING (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "Admins can manage support tickets" ON public.support_tickets FOR ALL USING (public.is_admin());
